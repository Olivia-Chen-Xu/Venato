    ...
    return (
        <>
            {title && (
                <div className={`w-full bg-white pb-4 px-${margin} flex-[0_1_auto]`}>
                    <div className={`${margin ? margin : 'mx-10'}`}>
                        {isLoading ? (<Skeleton animation="wave" sx={{
                                fontSize: '1.875rem',
                                lineHeight: '2.25rem',
                                width: '25%'
                            }}/>)
            ...
            <div className={classes} style={styles} {...rest} >
                {isLoading ? (
                    <div className="flex justify-center items-center h-full"><CircularProgress/>
                    </div>) : (
                    <>
                        {children}
                        {isEmpty && <div
                            className='flex justify-center content-center my-[7rem]'>{empty}</div>}
                    </>
                )}
            </div>
        </>
